...
func mealReturnFunc(meal:String)->Meal!{
    return meals[meal]
}
if let mealLog=mealReturnFunc(meal: "Breakfast"){
    print(mealLog.food)
}else{
    print("user hasn't logged that meal")
}
if let mealLog=mealReturnFunc(meal: "Lunch"){
    print(mealLog.food)
}else{
    print("user hasn't logged that meal")
}
