...
var user1=RunningWorkout(distance: 1800, time: 45, elevation: 89.6)
print(user1.averageMileTime())
...
struct Steps {
    var steps: Int{
        willSet{
            print("the value of steps is going to increment")
        }didSet{
            if steps == goal{
                print("congratulate you arive on your goal")
            }else{
                print("its only few steps away \(goal-steps)")
            }
        }
    }
    var goal: Int
...
var user=Steps(steps: 9998, goal: 10000)
user.takeStep()
user.takeStep()
/*:
